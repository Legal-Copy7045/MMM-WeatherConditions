/**
 * OpenWeather One Call 3.0 source — needs a free OWM API key (One Call 3.0
 * must be enabled on the account, card required by OWM even on the free
 * 1,000 calls/day tier). At a 10-minute poll interval that's ~144 calls/day,
 * comfortably inside the free allowance.
 */

function buildUrl({ lat, lon, apiKey, units = "metric", lang = "en" }) {
  const params = new URLSearchParams({ lat, lon, appid: apiKey, units, lang });
