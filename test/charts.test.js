...
test("soilChartConfig shares the daily chart's temp axis and tick styling, with no precip axis", () => {
  const state = { soilForecast: [{ time: "2026-01-01T00:00:00Z", tempC: 8 }] };
  const config = { ...baseConfig(), soilForecastDays: 1 };
  const dailyCfg = charts.dailyChartConfig({ daily: [{ date: "2026-01-01T00:00:00Z", tempMaxC: 10, tempMinC: 2, precipMm: 0, windKmh: 10 }] }, config);
  const soilCfg = charts.soilChartConfig(state, config);
  assert.deepEqual(soilCfg.options.scales.x, dailyCfg.options.scales.x);
  assert.deepEqual(soilCfg.options.scales.temp, dailyCfg.options.scales.temp);
  assert.equal(soilCfg.options.scales.precip, undefined);
  assert.equal(soilCfg.data.datasets[0].yAxisID, "temp");
});

test("soilChartConfig labels its line's points like the daily temperature line", () => {
  const state = { soilForecast: [{ time: "2026-01-01T00:00:00Z", tempC: 8 }] };
  const cfg = charts.soilChartConfig(state, { ...baseConfig(), soilForecastDays: 1 });
  assert.ok(cfg.plugins.some((p) => p.id === "wcPointLabels_Soil temp"));
});
